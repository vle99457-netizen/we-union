...
try {
  ...
  assert(await page.$$eval('.admin-sidebar nav button', (buttons) => buttons.length === 10), 'Admin navigation must expose ten modules.')
  ...
  await page.waitForFunction(() => document.querySelector('[data-admin-section-image="supporting-communityGalleryPrimary"] .admin-image-field__message.is-success')?.textContent?.includes('Image uploaded'))
  assert(await page.$eval('[data-admin-section-image="supporting-communityGalleryPrimary"] .admin-image-field__preview img', (image) => image.getAttribute('src') === '/images/world-honor.webp'), 'Completed section uploads must update the managed preview.')
  assert(await page.$eval('.admin-publish-bar', (bar) => bar.dataset.dirty === 'true'), 'A section image upload must remain unpublished until the publish action is used.')
  await page.screenshot({ path: '/tmp/we-admin-section-images-en.png', type: 'png', fullPage: true })
  ...
  await page.click('.admin-sidebar nav button:nth-child(4)')
  await page.waitForFunction(() => document.querySelector('.admin-topbar h1')?.textContent === '栏目图片')
  assert(await page.$eval('.admin-section-image-grid', (grid) => getComputedStyle(grid).gridTemplateColumns.split(' ').length === 1), 'Section image controls must stack into one column on mobile.')
  assert(await page.$eval('body', (body) => body.scrollWidth <= window.innerWidth), 'Mobile admin must not overflow horizontally.')
  await page.screenshot({ path: '/tmp/we-admin-section-images-mobile.png', type: 'png', fullPage: true })
  ...
  console.log('/tmp/we-admin-section-images-mobile.png')
} finally {
  await browser.close()
  await server.close()
